...
export function registerReportTools(
  ...
  // -----------------------------------------------------------------------
  // Audit log (Phase 14 #66 — qb_audit_log)
  // -----------------------------------------------------------------------
  // Enterprise-only — QB Desktop's audit-trail report is gated by edition.
  // Pro / Premier / Accountant editions return statusCode 9003 with a clear
  // edition-detected message; Enterprise / EnterpriseAccountant proceed.
  //
  // Args: exactly one of `txnId` or `dateRange` (XOR). Both supplied → 3120.
  // Neither supplied → 3120. Mirrors how operators query in practice ("show
  // me the audit history for THIS txn" OR "what changed during this window?").
  //
  // Wire: CustomDetailReportQueryRq with CustomDetailReportType=AuditTrail.
  // The wire-level report has NO TxnIDFilter, so txnId scope is implemented
  // by the tool layer post-fetch — for txnId-only scope we default the
  // wire-side period to a 2-year lookback (matches QB Desktop's Audit Trail
  // report default range) and filter rows by TxnID locally. Cost noted: on a
  // very-large audit log a 2-year fetch may be slow; pass a tight dateRange
  // alongside the txnId (handled as "both args" → 3120 today; consider
  // relaxing to AND-combine if operator workflows demand it).
  //
  // Output shape: `{ count, entries: [{user, timeModified, modifyType,
  // changedField, oldValue, newValue, txnId, txnType}] }`. Sorted desc by
  // timeModified (most-recent-first; matches QB Desktop's UI default). Entries
  // for Added / Deleted modifyTypes have empty changedField / oldValue /
  // newValue (no per-field diff applies to whole-entity events).
  //
  // Live verification deferred — sim emits canonical column names; live QB's
  // exact column titles may differ. If live returns missing-column data, the
  // fix is in `runAuditTrailReport`'s IncludeColumn list, not the row mapper
  // (which reads columns by title and tolerates missing keys via "" defaults).
  server.tool(
    "qb_audit_log",
    "Read QB Desktop's audit trail — per-field modification events for transactions (who changed what, when, from what value to what). ENTERPRISE-ONLY: Pro / Premier / Accountant editions reject with statusCode 9003 (audit-trail report is gated by QB edition; no SDK workaround). Pass exactly one of `txnId` OR `dateRange` (both → 3120, neither → 3120). txnId scope fetches a 2-year lookback window and filters by TxnID post-fetch (QBXML's AuditTrail report has no TxnIDFilter at the wire level — server-side scoping is date-only). dateRange scope passes through to the wire ReportPeriod as-is. Returns `{ count, entries: [{user, timeModified, modifyType, changedField?, oldValue?, newValue?, txnId, txnType}] }` sorted desc by timeModified (most-recent-first). Added / Deleted events omit changedField / oldValue / newValue (whole-entity events have no per-field diff). Use for compliance review (who modified this invoice?), forensic accounting (when did this transaction's amount change?), and the standard 'show me what changed last week' question. NOT a write tool — audit entries are auto-generated by QB on every mutation; this server's mutations also generate entries in real QB but the SDK doesn't expose a write path to the audit log itself.",
    {
      txnId: z.string().optional().describe("Scope to a single transaction by TxnID. Returns every audit entry across the last 2 years that targets this TxnID (post-filter — QB's AuditTrail report has no TxnIDFilter at the wire). Mutually exclusive with `dateRange` — supplying both rejects with statusCode 3120."),
      dateRange: z
        .object({
          fromDate: z.string().regex(ISO_DATE_RE, "fromDate must be YYYY-MM-DD"),
          toDate: z.string().regex(ISO_DATE_RE, "toDate must be YYYY-MM-DD"),
        })
        .optional()
        .describe("Scope by modification window. Both fromDate + toDate required (ISO YYYY-MM-DD). Filters server-side via ReportPeriod against TimeModified. Mutually exclusive with `txnId` — supplying both rejects with statusCode 3120."),
    },
    async (args) => {
      const session = getSession();

      // XOR validation — both or neither rejects with 3120.
      if (args.txnId && args.dateRange) {
        return {
          content: [{
            type: "text" as const,
            text: JSON.stringify({
              ...
              statusCode: 3120,
              statusMessage:
                "qb_audit_log accepts exactly one of `txnId` or `dateRange` — both supplied. Pass one and retry.",
        ...
        return {
          content: [{
            ...
            text: JSON.stringify({
              ...
              statusCode: 3120,
              statusMessage:
                "qb_audit_log requires either `txnId` or `dateRange` — neither supplied. Pass one and retry.",
              humanReadable: qbStatusCodeMessage(3120),
            }),
          }],
          isError: true,
      ...
      // Edition gate — Enterprise-only. Pro / Premier / Accountant reject.
      try {
        const hostInfo = await session.getHostInfo();
          ...
          return {
            content: [{
              ...
              text: JSON.stringify({
                success: false,
                statusCode: 9003,
                statusMessage: `qb_audit_log requires QB Enterprise (audit-trail report is Enterprise-only); detected edition: ${hostInfo.edition} (productName: '${hostInfo.productName}')`,
                humanReadable: qbStatusCodeMessage(9003),
                edition: hostInfo.edition,
                productName: hostInfo.productName,
              }),
      ...
      // Resolve wire-side date window. dateRange → pass through. txnId-only →
      // default to a 2-year lookback (matches QB Desktop's Audit Trail UI default).
      let fromDate: string;
      let toDate: string;
      if (args.dateRange) {
        fromDate = args.dateRange.fromDate;
        toDate = args.dateRange.toDate;
      } else {
        const today = new Date();
      ...
      try {
        const ret = await session.runAuditTrailReport({ fromDate, toDate });
        const rows = (ret.Rows as Array<Record<string, unknown>> | undefined) ?? [];

        const allEntries = rows.map((r) => {
          const out: Record<string, unknown> = {
            user: String(r.User ?? ""),
            timeModified: String(r.TimeModified ?? ""),
            modifyType: String(r.ModifyType ?? ""),
            txnId: String(r.TxnID ?? ""),
            txnType: String(r.TxnType ?? ""),
          };
          // Only surface changedField / oldValue / newValue when populated —
          // Added / Deleted events have no per-field diff (whole-entity event).
          const changedField = r.ChangedField !== undefined && r.ChangedField !== null && String(r.ChangedField) !== ""
            ? String(r.ChangedField) : null;
          const oldValue = r.OldValue !== undefined && r.OldValue !== null && String(r.OldValue) !== ""
            ? String(r.OldValue) : null;
          const newValue = r.NewValue !== undefined && r.NewValue !== null && String(r.NewValue) !== ""
            ? String(r.NewValue) : null;
          if (changedField !== null) out.changedField = changedField;
          if (oldValue !== null) out.oldValue = oldValue;
          if (newValue !== null) out.newValue = newValue;
          return out;
        });

        // txnId scope: post-filter (QB's AuditTrail report has no TxnIDFilter).
        const entries = args.txnId
          ? allEntries.filter((e) => e.txnId === args.txnId)
          : allEntries;

        // Sort desc by timeModified — most-recent-first. Sim already returns
        // sorted rows but live row order is not guaranteed.
        entries.sort((a, b) => {
          const at = String(a.timeModified ?? "");
          const bt = String(b.timeModified ?? "");
          return at < bt ? 1 : at > bt ? -1 : 0;
        });
        ...
        return {
          content: [{
            ...
            text: JSON.stringify({
              success: true,
              scope: args.txnId ? { txnId: args.txnId } : { dateRange: { fromDate, toDate } },
              fromDate,
              toDate,
        ...
        return formatToolError(err, { fallbackMessage: "CustomDetailReportQueryRq (AuditTrail) failed" });
      }
    }
  );
