...
const server = new McpServer(
  ...
  },
  {
    ...
    instructions: [
      ...
      "",
      "Idempotency keys: every *_create / *_add tool (qb_customer_add, qb_vendor_add, qb_account_add, qb_employee_add, qb_item_add, qb_invoice_create, qb_invoice_duplicate, qb_invoice_write_off, qb_bill_create, qb_bill_duplicate, qb_bill_pay, qb_payment_receive, qb_estimate_create, qb_estimate_convert_to_invoice, qb_sales_receipt_create, qb_sales_receipt_duplicate, qb_credit_memo_create, qb_purchase_order_create, qb_journal_entry_create, qb_journal_entry_duplicate, qb_journal_entry_batch_create) accepts an optional idempotencyKey:string arg. Retrying with the same key + same payload returns the original result instead of duplicating the QB record (the response carries idempotentReplay:true). Same key + different payload returns statusCode 9002 — use a fresh key for new requests. The cache is in-memory, scoped per company file, and resets on qb_company_open. Failed creates are NOT cached (next retry can fix the underlying problem). For qb_journal_entry_batch_create the key fingerprints the entire entries list and only fully-successful batches are cached.",
      "",
      "Workflow tips:",
