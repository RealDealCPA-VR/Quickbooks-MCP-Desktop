 ...
 * - Sales Order management (customer-side, non-posting commitment)
 * - Sales tax workflows (codes, items, agencies, liability report, payment)
 * - Inventory adjustments (shrinkage, count corrections, value write-downs)
...
import { registerEngagementProfitabilityTools } from "./tools/engagement-profitability.js";
import { registerSalesOrderTools } from "./tools/sales-orders.js";
import { registerSalesTaxTools } from "./tools/sales-tax.js";
import { registerInventoryAdjustmentTools } from "./tools/inventory-adjustments.js";
...
const server = new McpServer(
  ...
  },
  {
    ...
    instructions: [
      ...
      "  • qb_sales_order_*  — Sales order management (list, create, update, delete, convert_to_invoice). Customer-side analog of qb_purchase_order_* — a non-posting commitment from the customer to buy (committed but not yet invoiced). DISTINCT FROM ESTIMATE: estimates are quotes (customer hasn't committed); sales orders carry committed-but-unfulfilled orders for backorder / fulfillment workflows. Does NOT move Customer.Balance / AR (that only moves when an invoice is created against the order). Lines use Rate (sales side, mirrors qb_invoice_create); the simulation derives TotalAmount = sum(line amounts). qb_sales_order_create requires at least one line. qb_sales_order_update mirrors qb_invoice_update for header/line edits — passing `lines` REPLACES the line set wholesale and TotalAmount recomputes; isManuallyClosed:true marks the order closed (typical: cancel an order that won't ship), false reopens it. Stale editSequence rejects with 3170. qb_sales_order_convert_to_invoice reads the source order, submits an InvoiceAddRq with CustomerRef + carried lines (and optional ClassRef / TermsRef / SalesRepRef / PONumber from the order header), and (default) marks the source order IsManuallyClosed=true; pass markClosed:false for partial conversions where another invoice will be billed against the remaining lines. The mark-closed step runs after the invoice is created, so the new invoice is preserved even if the flip fails (rare — surfaced via markClosedError in the response).",
      ...
      "  • qb_time_track_* — Time tracking primitives (Phase 17 #78). qb_time_track_add records ONE work session — a date, a worker (Employee/Vendor/OtherName via entityName | entityListId — this is WHO did the work, not the customer), an optional customer/job (customerName | customerListId), an optional service item (itemServiceName | itemServiceListId), and a duration. Duration is supplied either as decimal `hours` (e.g. 8.5 → PT8H30M) OR as ISO 8601 `duration` (PT-H-M-S — overrides hours when both supplied). billable:true sets IsBillable + BillableStatus='Billable' for downstream Time/Costs invoice flows; default unset = NotBillable. payrollItemWageName links the timesheet to a paycheck wage item (requires a payroll subscription on live QB; sim records the ref verbatim). TimeTracking is NON-POSTING — no GL effect, no AR/AP movement — so it doesn't appear in any report tool's section walks; it's a write-once payload consumed by payroll and Time/Costs invoicing. qb_time_track_list returns rows with a derived `hours` field (parsed from Duration) for convenience. Filter by txnId / entityName / entityListId (worker — applied server-side via EntityFilter) / customerName / customerListId (POST-FILTERED — QB's TimeTrackingQueryRq has no CustomerFilter, so combining with paginate may miss matches beyond the first page) / fromDate / toDate / billableOnly (also POST-FILTERED). idempotencyKey on add — replay → idempotentReplay:true, key+different-payload → 9002. Read-only sessions reject add with 9001.",
      ...
      "  • qb_inventory_adjustment_* — Inventory adjustments (Phase 17 #80). Three tools (list / create / delete) for shrinkage, count corrections, write-offs of damaged stock, value write-downs and write-ups. Each line picks one of three shapes: (a) pure quantity adjustment via newQuantity (absolute) OR quantityDifference (signed delta) — value moves at current AverageCost; (b) pure value adjustment via newValue OR valueDifference — repricing without changing count; (c) combined value+qty adjustment. accountName is REQUIRED — the offsetting GL account (typically 'Inventory Adjustment' expense, 'Cost of Goods Sold', 'Shrinkage'). Sim mutates ItemInventory.QuantityOnHand / QuantityOnHandValue / AverageCost on every referenced item; AverageCost recomputes from post-adjustment value/qty (preserved at zero qty so cost-basis history survives a stock-out). Two-phase commit — a malformed line in position 5 won't leave items 1-4 partially mutated. Delete reverses every line's qty/value delta against the still-present ItemInventory row (orphan items silently skipped). On query, real QB normalizes both input shapes — every line returns QuantityDifference + ValueDifference regardless of which input form was used; sim mirrors. TotalAmount = Σ ValueDifference (negative on shrinkage). NO _update tool — operators delete + recreate when an adjustment needs to change. Enterprise-only fields (SerialNumber, LotNumber, InventorySite refs) intentionally NOT exposed. Read-only sessions reject create/delete with 9001. idempotencyKey on create (replay → idempotentReplay:true, key+different-payload → 9002). GL posting to AccountRef is NOT modeled in sim's first cut (matches deferred JE-line customer-balance behavior).",
      "  • qb_sales_tax_* — Sales tax workflows (Phase 17 #77). Five tools covering the monthly sales-tax cycle for any client with taxable sales. qb_sales_tax_code_list reads SalesTaxCodeQueryRq — short flags (TAX/NON/OUT) stamped on customers and lines to indicate taxable status; codes carry NO rate, just IsTaxable. qb_sales_tax_item_list reads ItemSalesTaxQueryRq + ItemSalesTaxGroupQueryRq — items carry TaxRate + TaxVendorRef (the agency); groups bundle items so a single line collects state + county at once. Default fans across both; pass taxItemType to scope. Each row carries an `ItemType` discriminator ('SalesTaxItem' | 'SalesTaxGroup'). qb_sales_tax_agency_list derives the agency set from distinct ItemSalesTax.TaxVendorRef values (real QB has NO separate SalesTaxAgency entity — agencies are just Vendors), enriches each with full Vendor details by default + the list of tax items that pay into it. Pure composite over qb_sales_tax_item_list + per-agency Vendor lookups (N round trips with includeVendorDetails:true; pass false to skip). qb_sales_tax_liability_report wraps GeneralSummaryReportType=SalesTaxLiability — one row per SalesTaxItem with TaxCollected (Invoice + SalesReceipt header SalesTaxTotal scoped to that item, minus CreditMemo returns), TaxPaid (SalesTaxPaymentCheckLineRet sums in window), TaxPayable (= TaxCollected − TaxPaid). Per-agency rollups + grand totals. Sim mode tracks tax-collection at the HEADER level only (txn.ItemSalesTaxRef + txn.SalesTaxTotal) — per-line tax flagging is not modeled; for the typical month-end question 'what do I owe each agency?' the header-level model suffices. qb_sales_tax_payment_create wraps SalesTaxPaymentCheckAddRq — DISTINCT FROM qb_check_create because lines reduce sales-tax-item liability instead of posting to expense GL (a regular Check posted to a tax-liability account would double-count). At least one line required; each names ONE SalesTaxItem + Amount; sum is drawn from bankAccountName. payeeName is the tax agency vendor (typically matches the agency on the lines' SalesTaxItem.TaxVendorRef — real QB validates this server-side; sim doesn't). Bank-affecting (defaults to ClearedStatus: NotCleared; participates in qb_uncleared_transactions). idempotencyKey on create (replay → idempotentReplay:true, key+different-payload → 9002). Read-only sessions reject create with 9001.",
