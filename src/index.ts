...
const server = new McpServer(
  ...
  },
  {
    ...
    instructions: [
      ...
      "  • qb_cleared_status_update — Bank-reconciliation primitive (wraps ClearedStatusModRq). Marks a bank/CC transaction Cleared / NotCleared / Pending. Targets the seven bank-affecting types only (Check, BillPaymentCheck, BillPaymentCreditCard, Deposit, Transfer, CreditCardCharge, CreditCardCredit); calls against Invoice/Bill/JE/etc. return statusCode 3120 because those headers don't carry cleared status. Pass txnId for whole-transaction update; add txnLineId for single-line update of a split (multi-account Deposit / multi-expense Check). Naturally idempotent — flipping Cleared on an already-Cleared txn is a server no-op, so no idempotencyKey arg. Read-only sessions reject with 9001. Workflow: pair with QB Desktop's reconciliation screen — UI surfaces the bank statement match candidates, this tool flips them Cleared in agent-driven bulk. Read-side (which txns are uncleared) needs CustomDetailReportQueryRq infrastructure landing in Phase 11 alongside #56.",
      ...
      "",
      "Idempotency keys: every *_create / *_add tool (qb_customer_add, qb_vendor_add, qb_account_add, qb_employee_add, qb_item_add, qb_invoice_create, qb_invoice_duplicate, qb_bill_create, qb_bill_pay, qb_payment_receive, qb_estimate_create, qb_estimate_convert_to_invoice, qb_sales_receipt_create, qb_credit_memo_create, qb_purchase_order_create, qb_journal_entry_create, qb_journal_entry_batch_create) accepts an optional idempotencyKey:string arg. Retrying with the same key + same payload returns the original result instead of duplicating the QB record (the response carries idempotentReplay:true). Same key + different payload returns statusCode 9002 — use a fresh key for new requests. The cache is in-memory, scoped per company file, and resets on qb_company_open. Failed creates are NOT cached (next retry can fix the underlying problem). For qb_journal_entry_batch_create the key fingerprints the entire entries list and only fully-successful batches are cached.",
      "",
      "Workflow tips:",
