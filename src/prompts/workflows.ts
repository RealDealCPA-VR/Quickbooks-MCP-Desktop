...
export const trialBalanceWorkupPrompt: PromptCallback<typeof trialBalanceArgs> = (args) => {
  ...
  const text = [
    ...
    "",
    "Pull a TB + adjacent cross-checks for the operator's trial-balance workpaper (same shape as the existing `trial-balance-workup` skill). The new `qb_trial_balance_export` tool bundles the entire TB + cross-checks into one call — earlier versions of this prompt walked 8 tools to assemble the same view.",
    "",
    "## 1. One-call trial balance + cross-checks",
    `- \`qb_trial_balance_export({ asOfDate: "${asOfDate}", basis: "${basis}", includeLastActivityDate: true })\` — returns:`,
    "  - **rows**: one per posting account with non-zero balance, debits/credits split by natural-balance side (Asset/Expense → debit column; Liability/Equity/Income → credit column; contra-balances flip column). Sorted by canonical AccountType then AccountNumber then name. Each row carries `accountName` / `accountType` / `accountNumber` / `taxLine` / `debitBalance` / `creditBalance` / `isActive` / `lastActivityDate`.",
    "  - **totals**: `totalDebits` / `totalCredits` / `isBalanced` / `delta`. `isBalanced: false` is the TB-doesn't-foot signal — investigate before publishing.",
    "  - **crossChecks**: four reconciliations done server-side, each with delta + match boolean (cent tolerance):",
    "    - `balanceSheet`: Assets ≡ Liabilities + Equity (the accounting identity).",
    "    - `netIncome`: P&L NetIncome ≡ BS NetIncome (period income closes into equity correctly).",
    "    - `arReconciliation`: TB AR account total ≡ AR aging total (subledger ties to GL).",
    "    - `apReconciliation`: TB AP account total ≡ AP aging total (subledger ties to GL).",
    "  - Any `matches: false` / `reconciles: false` / `isBalanced: false` is an audit signal — surface loudly, do not paper over.",
    "",
    "  *Note: `includeLastActivityDate: true` triggers a per-account TransactionQueryRq fanout (~10s for a 200-account chart in live mode). Drop the flag for a quick balance-only TB.*",
    "",
    "## 2. Drill-downs (operator-driven, only when needed)",
    ...
    `- \`qb_general_ledger({ toDate: "${asOfDate}", accountType: <type> })\` — full GL detail by AccountType (Expense / Income / etc.).`,
    "- `qb_customer_balance_detail({ customerName })` or `qb_vendor_balance_detail({ vendorName })` for AR / AP subledger drill.",
    "",
    "## 3. Output to the operator",
    "Surface the rows from step 1 as a single workpaper table:",
    "",
    "| AccountNumber | AccountName | AccountType | TaxLine | DebitBalance | CreditBalance | LastActivityDate |",
    "",
    "Followed by a short cross-checks block (one line per reconciliation, marking ✓ for matches and the dollar delta for misses).",
    "",
    "If any cross-check fails, halt and propose investigation steps before producing the final workpaper — a TB that doesn't tie is not ready to publish.",
  ].join("\n");
  return userTextResult(
