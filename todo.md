...
- [x] **88.** ~~Publish to npm~~ — **CLOSED 2026-05-24 NOT-PURSUED.** Distribution is via the private GitHub repo only — buyers are granted access by being added as repo collaborators, then install with `npx -y github:RealDealCPA-VR/Quickbooks-MCP-Desktop` (works today via #87's `bin` + `prepare` wiring). Publishing privately to npm would require an npm Pro plan ($7/mo) for a scoped `@realdealcpa/quickbooks-desktop-mcp` private package — the private GitHub repo already gates access effectively, so the npm tier is redundant cost. Publishing publicly is rejected because the operator wants to sell access. **Side effect:** the README §2 install-path table no longer lists an "npm" row; the `github:RealDealCPA-VR/...` form is now the permanent install path, not a provisional one. DECISIONS.md 2026-05-24 pins the distribution-model decision.
...
- [x] **90.** Auto-launch QB Desktop on `qb_company_open` — `launchIfClosed: boolean` (default false; explicit opt-in). When true + live mode + initial `openSession` fails with a `file-not-loaded` signal: detect QB Desktop exe via the fallback chain (`$QB_DESKTOP_EXE` → Windows registry `HKLM\SOFTWARE\Intuit\QuickBooks*\InstallPath` → known Program Files paths), spawn with the `.qbw` as a process arg (detached, unref'd so QB survives the Node exit), poll `openSession` against the canonical [1s, 2s, 4s, 8s, 15s] schedule (30s total budget). New synthetic statusCodes: **9007** (file-conflict / no-executable / launch-spawn-failed / launch-timeout — operator-resolvable) + **9008** (multi-user-lock — wait for release). Sim mode no-ops the flag (existing reseed already covers "open a new book"). _(Closed 2026-05-28. [src/util/qb-desktop-launch.ts](src/util/qb-desktop-launch.ts) owns exe detection + error classifiers + the spawn primitive; orchestration in `QBSessionManager.attemptLaunchAndAttach`; tool surface gains `launched/launchSource/launchExe/launchPollAttempts` fields on success. Design Qs resolved 2026-05-28: (a) fallback chain; (b) fail-fast on file-conflict — no UI automation; (c) 9008 + remediation hint, no retry; (d) sim no-op. DECISIONS.md 2026-05-28. Live verification deferred — spawn + QBXMLRP2 attach behavior cannot be exercised without a Windows + QB Desktop box. 43 net new tests; total 1690.)_
